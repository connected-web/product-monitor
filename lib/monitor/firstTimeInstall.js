...
function checkForUserDirectory () {
  ensureExists(userContentPath, 744, function (error, folderCreated) {
    if (error || folderCreated) {
...
function checkForContentDirectory () {
  var folder = '/content'
  ensureExists(userContentPath + folder, 744, function (error, folderCreated) {
    if (error || folderCreated) {
...
function ensureExists (path, mask, cb) {
  ...
  try {
    ...
    if (error && error.code === 'EEXIST') {
      ...
      cb(error, false) // something else went wrong
    }
  }
}
