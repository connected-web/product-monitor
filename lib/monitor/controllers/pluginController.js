...
function configure (config) {
  ...
  function loadPlugins (app, path, storageModel) {
    return find(path + '/*.json')
      ...
      .catch((ex) => console.log('[Plugin Controller]', ex, ex.stack))
  }
  ...
  function localize (entry) {
    const library = entry.plugin.library || 'undefined'
    const localPath = (/^(local\.).*/).test(library) ? library.replace('local.', entry.info.dir + '/') + '/' : library
    return localPath
  }
  ...
  function applyPlugin (entry, app) {
    if (entry.error) {
      ...
      try {
        ...
        console.log('[Plugin Controller] Stack trace:', ex.stack)
        entry.error = {
      ...
      try {
        ...
        console.log('[Plugin Controller] Loading', libraryPath)
