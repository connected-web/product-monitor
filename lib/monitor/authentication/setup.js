...
function setup(app) {
  ...
  function enableAuthentication(mode) {
    ...
    server.get('/auth/status', function (req, res) {
      var user = sanitize(req.user);
      var roles = listRolesFor(user);
      var supportedModes = authenticationModel.supportedModes;
      res.jsonp({
        enabled: true,
        user,
        roles,
    ...
    function broker(req, res) {
      ...
      var params = (req.query.redirect) ? '?redirect=' + req.query.redirect : '';
...
function registerContentTemplates(app) {
  ...
  app.addContentPage(__dirname + '/pages/user-details.fragment.html');
}
