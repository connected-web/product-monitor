...
instance.configure = function (config) {
  ...
  apicache = require('apicache').options(config.apiCache).middleware;
...
function registerApi(file) {
  try {
    ...
    // Retrieve duration from the module
    var cacheDuration = apiModule.cacheDuration || "1 hour";

    // Register module as express server route
    server.get(apiModule.route, apicache(cacheDuration), apiModule.render);
  }
  catch(e) {
