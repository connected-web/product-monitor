...
function registerApiFiles(files, description) {
  var fileRoutes = files.map(registerApi);
...
function registerApi(file) {
  try {
    ...
    console.log("[API Warning] Unable to register API, error: " + e + NL + e.stack);
  }
...
instance.registerServerAPIs = function () {
  ...
  registerApiFiles(serverFiles, 'Boxed routes');
  return this;
}
...
instance.registerUserAPIs = function () {
  ...
  registerApiFiles(serverFiles, 'User routes');
  return this;
}
