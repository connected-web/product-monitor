...
function configureRoutes(instance, apiController, pageController) {
  ...
  // Parse application/x-www-form-urlencoded from form data
  ...
  return find(documentationPattern)
    .then((files) => files.map(instance.addContentPage))
    .then(function (routes) {
      console.log('[Navigation] Boxed content:', NL + ' ', routes.join(NL + '  '), NL);
    })
    .catch(function (ex) {
      console.error('[Navigation] Unable to find documentation fragments', ex);
    });
}
...
function startServer(config, onReady) {
  ...
  var pageController = require('./monitor/pageController').configure(config);

  // Create API controller
  var apiController = require('./monitor/apiController').configure(config);

  // Create Plugin controller
  var pluginController = require('./monitor/pluginController').configure(config);
  ...
  firstTimeInstall.runChecks(function () {
    // Register local content routes routes
    configureRoutes(instance, apiController, pageController)
      .then(() => pluginController.loadPlugins(instance))
      .then(onConfigured)
      .catch(console.log);
  });
