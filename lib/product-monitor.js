...
function startServer(server, instance, config, onReady) {
  ...
  instance.listen = function (onListening) {
    return heyListen(server, config, onListening);
  };
...
function heyListen(server, config, onListening) {
  return server.listen(config.serverPort, config.ipAddress, function () {
      console.log(
...
module.exports = function (config, onReady) {
  ...
  firstTimeInstall.runChecks(function () {
    startServer(server, instance, config, onReady);
  });
